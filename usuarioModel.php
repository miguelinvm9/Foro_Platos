<?php
    ...
    function actualizarUsuario($id, $nombre_usuario, $nombre, $apellidos, $contrasena, $esAdmin, $correo, $experiencia, $etiqueta_usuario) {
        global $conexion;
		$sql = "UPDATE usuario SET nombre_usuario = :nombre_usuario, nombre = :nombre, apellidos = :apellidos, contrasena = :contrasena, esAdmin = :esAdmin, correo = :correo, experiencia = :experiencia, etiqueta_usuario = :etiqueta_usuario WHERE id = :id";
        $stmt = $conexion->prepare($sql);
    ...
    // Método para desvincular un usuario por su ID de las recetas
    function hacerAdminUsuarioPorId($id) {
        global $conexion;
		$sql = "UPDATE usuario SET esAdmin = 1 WHERE id=:id";
        $stmt = $conexion->prepare($sql);
        $stmt->bindParam(':id', $id);
    ...
    // Método para desvincular un usuario por su ID de las recetas
    function deshacerAdminUsuarioPorId($id) {
        global $conexion;
		$sql = "UPDATE usuario SET esAdmin = 0 WHERE id=:id";
        $stmt = $conexion->prepare($sql);
        $stmt->bindParam(':id', $id);
    ...
    // Método para desvincular un usuario por su ID de las recetas
    function eliminarUsuarioRecetaPorID($id) {
        global $conexion;
		$sql = "UPDATE receta SET id_usuario = 1 WHERE id_usuario=:id";
        $stmt = $conexion->prepare($sql);
        $stmt->bindParam(':id', $id);
